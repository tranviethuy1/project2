...
<body>
	...
	<div clas="row">
		<p class ="text-info" style="text-align: center;">PHIẾU TẠM ỨNG </p>
	</div>
		      	...
		      	<tr>
			        ...
			        <td>
			        	<ul class="project">
			        		...
			        		<li>Date: {{$advance->advance_date}}</li>
			        		<li>Travel : {{number_format($advance->travel_cost)}}</li>
			        		<li>Rent: {{number_format($advance->rent_house)}}</li>
			        		<li>Postage: {{number_format($advance->postage)}}</li>
			        		<li>Document: {{number_format($advance->postage_document)}}</li>
			        		<li>Others: {{number_format($advance->others)}}</li>
			        		<hr>
			        		<li style="color:#007fff;">
			        			<?php 
			        				$total = $advance->travel_cost+$advance->rent_house+$advance->postage+$advance->postage_document+$advance->others; 
			        				$all+= $total;
			        			?>
