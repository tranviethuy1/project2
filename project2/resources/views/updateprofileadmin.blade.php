...
<body>
  ...
  <div class="container">
    <div class="row">
      <div class="col-md-10 ">
        <form class="form-horizontal" action="{{URL::Route('excuteupdateadmin',array($id))}}" method="post" enctype="multipart/form-data">
        {!! csrf_field() !!}
        <fieldset>
        <!-- Form Name -->
          <div class="status">
            @if(isset($error))
            <span class="alert alert-danger form-control" style="line-height: 10px;margin: auto; ">{!! $error !!}</span>
            @endif
          </div>
        ...
        <div class="form-group">
          ...
          <a href="{{URL::Route('admin')}}" class="btn btn-danger" value="">Back to home<i class="fas fa-arrow-circle-right"></i></a> 
          </div>
        </div>
